 ...
 * application client in response to dataSource.ConnectToServer().
 * 
...
public class ConnectionWrapper extends WrapperBase implements JdbcConnection {
     ...
     * object is de-referenced by the pooled connection each time mysqlPooledConnection.ConnectToServer() is called by app server.
     * 
     * @throws SQLException
     *             if an error occurs
     */
    ...
    protected void close(boolean fireClosedEvent) throws SQLException {
        synchronized (this.pooledConnection) {
            ...
            // re-used by the pooled connection each time ConnectToServer is called.
            this.closed = true;
        }
    }
